...
class ProductGrid:
    ...
    def __init__(self, model_obj, obs_obj, model, obs):
        ...
        self._obs_data = obs_obj.data[obs][:]
    ...
    def generate_regrid_product(self):
    ...
    def _product_method_storage(self):
        dict = {f'{self._obs}': np.zeros(self._model_height.shape)}
    ...
    def _append_data2object(self, array_list):
        for array_dict in array_list:
            for key in array_dict.keys():
                storage = array_dict[key]
                self.model_obj.append_data(storage, f"{key}_{self._model}{self.model_obj._cycle}")
