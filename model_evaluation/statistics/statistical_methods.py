...
class DayStatistics:
    """ Class for calculating statistical analysis of day scale products
            ...
            Args:
                ...
                model_array (np.array): Ndarray of model simulation of product
                obs_array (np.array): Ndrray of Downsampled observation of product
    ...
    def _get_method_attr(self) -> Tuple[str, tuple]:
        ...
        if self.method == 'hist':
            full_name = 'histogram'
            params = (self.product, self.model_data, self.observation_data)
...
def absolute_error(model: ma.array, observation: ma.array) -> Tuple[float, str]:
    model, observation = combine_masked_indices(model, observation)
...
def calc_common_area_sum(model: ma.array, observation: ma.array) -> Tuple[float, str]:
...
def vertical_profile(model: ma.array, observation: ma.array) -> Tuple:
    if model.shape[0] > 25:
        model = model.T
